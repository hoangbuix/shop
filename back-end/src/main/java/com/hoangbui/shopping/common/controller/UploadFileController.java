...
public class UploadFileController {
    ...
    public ResponseEntity<List<FileInfo>> getListFiles() {
        List<FileInfo> fileInfos = fileStoreService.loadAll().map(path -> {
            ...
            String url = MvcUriComponentsBuilder
                    .fromMethodName(UploadFileController.class, "getFile", path.getFileName().toString()).build()
                    .toString();
    ...
    public String listUploadedFiles(Model model) throws IOException {
        model.addAttribute("files", fileStoreService.loadAll().map(path -> MvcUriComponentsBuilder.fromMethodName(
                UploadFileController.class, "serveFile", path.getFileName().toString()).build().toUri().toString())
                .collect(
                        Collectors.toList()));
        return "uploadForm";
    }
